...
| TLUT (`J2D 'TLUT'`) | `JUTPalette::storeTLUT` | `ResTLUT` | in place | none | not converted yet (no `.bpl`-style files met on the title/menu path); swap `u16` at +2 when needed |
...
| BMS sequences | `JASSeqCtrl`, `JASSeqParser`, `JASTrack` | byte code | byte-wise shift composition (endian-safe) | – | **remaining**: s16 tables referenced by opcodes 0xD7 (`cmdSimpleEnv`), 0xF2 (`cmdOscFull`), 0xED (`cmdFIRSet`) are read in place big-endian; needs a pre-scan or BE loads in `JASTrack`/`JASOuterParam` |
...
| Scene/table .bin (`scene.bin`, `tables.bin`, `PerformLists.bin`, `stageArc.bin`, …) | `JDrama` `TNameRef` loaders via `JSUMemoryInputStream` | stream layout | typed reads | none | **kept big-endian** (0013). Remaining: raw 4-byte `stream.read(&x, 4)` sites in `Animal/BeeHive.cpp`, `Animal/Bird.cpp`, `JDrama/JDRViewport.cpp`, `Enemy/tobiPuku.cpp`, `Enemy/hanasambo.cpp`, `Enemy/wireTrap.cpp`, `MoveBG/MapObjBianco.cpp`, `MoveBG/MapObjMare.cpp`, `MoveBG/MapObjRicco.cpp` |
...
| THP movies | `THPPlayer.c`, `THPRead.c`, `THPAudio.c`, `THPDec.c` | `THPHeader`, `THPFrameCompInfo`, `THPAudioRecordHeader` | streamed from disc into buffers | none | **remaining**: swap the header/component info after `THPPlayerOpen`'s reads, the frame size words after each `DVDReadPrio` in `Reader`, the audio record header; the video bitstream must be read as big-endian u32 words by the decoder (never pre-swapped) |
| Memory card save | `FlagManager::save` writes raw, `CardManager.cpp:202` reads typed | – | stream | – | **remaining** (not a disc format): writes are raw, reads convert since 0013, so they disagree |
...
- `MarioUtil/DrawUtil.cpp:744` reads `*(u16*)(p + 1)` from a J3D display list (vertex count); display lists stay big-endian, so this needs a big-endian load.
