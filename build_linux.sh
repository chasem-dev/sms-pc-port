#!/usr/bin/env bash
# ./build_linux.sh [/path/to/GMSE01.iso]   (SMS_ARCH=64 for the 64-bit build in build-64/)
...
arch="${SMS_ARCH:-32}"
if [[ "$arch" == 64 ]]; then bdir=build-64; else bdir=build; fi
...
cmake -S . -B "$bdir" -DSMS_ARCH="$arch" -DSMS_GX_BUILD_TESTS=OFF -DSMS_BUNDLE_DISC="$disc"
